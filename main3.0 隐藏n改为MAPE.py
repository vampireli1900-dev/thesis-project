# ==============================================================================
# 光伏功率预测终极代码：全局多模型对比 + 局部最优100点精细化分析 (MAPE + 隐式归一化版)
# ==============================================================================
import pandas as pd
import numpy as np
...
def calc_normalized_metrics(y_true, y_pred):
    ...
    n_rmse = (rmse / capacity) * 100  # 本质是归一化计算，名字后面显示为 RMSE(%)

    return n_rmse, mape, r2
...
mapes = [results[m][1] for m in model_names]
...
plot_bar(model_names, mapes, '各模型 MAPE 对比 (%)', 'MAPE (%)', '3_MAPE对比.png')
...
# 5. 归一化误差柱状图 (标题和Y轴去除 'n')
fig_norm, axes_norm = plt.subplots(1, 3, figsize=(18, 5))

# 子图1: RMSE (%)
axes_norm[0].bar(models, df_best100["局部RMSE(%)"], color=colors, width=0.5)
for i, v in enumerate(df_best100["局部RMSE(%)"]):
    axes_norm[0].text(i, v + 0.1, f'{v:.2f}', ha='center', fontsize=11)
axes_norm[0].set_title('最优100点局部 RMSE (%)', fontsize=14)
axes_norm[0].set_ylabel('RMSE (%)')

# 子图2: MAPE (%)
axes_norm[1].bar(models, df_best100["局部MAPE(%)"], color=colors, width=0.5)
for i, v in enumerate(df_best100["局部MAPE(%)"]):
    axes_norm[1].text(i, v + 0.1, f'{v:.2f}', ha='center', fontsize=11)
axes_norm[1].set_title('最优100点局部 MAPE (%)', fontsize=14)
axes_norm[1].set_ylabel('MAPE (%)')

# 子图3: R2
axes_norm[2].bar(models, df_best100["局部R²"], color=colors, width=0.5)
for i, v in enumerate(df_best100["局部R²"]):
    axes_norm[2].text(i, v + 0.01, f'{v:.4f}', ha='center', fontsize=11)
axes_norm[2].set_title('最优100点局部 R²', fontsize=14)
axes_norm[2].set_ylabel('$R^2$')
axes_norm[2].set_ylim(0, 1.15)
